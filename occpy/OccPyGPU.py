...
from occpy.gpu_raytr import GPURaytracerWrapper
...
class OccPyGPU:
    def __init__(self, laz_in, out_dir, vox_dim=0.1, lower_threshold=1, points_per_iter=10000000, plot_dim=None,
                 output_voxels=False, use_gpu=False):
        """
        ...
        if use_gpu:
            self.RayTr = GPURaytracerWrapper()
            print("Using GPU Acceleration")
        else:
            self.RayTr = PyRaytracer()
            print("Using CPU Raytracer")
    ...
    def define_sensor_pos(self, path2file, is_mobile, single_return=None, delimiter=" ", hdr_time='%time',
                          hdr_scanpos_id='', hdr_x='x', hdr_y='y', hdr_z='z', sens_pos_id_offset=0, str_idx_ScanPosID=0,
                          str_end_idx_ScanPosID=0):
        """
        ...
        if is_mobile:  # case of mobile acquisitions (MLS, ULS)
            self.traj = read_trajectory_file(path2traj=path2file, delimiter=delimiter, hdr_time=hdr_time, hdr_x=hdr_x,
                                             hdr_y=hdr_y, hdr_z=hdr_z)
        else:  # case of static acquisition (TLS)
            self.senspos = read_sensorpos_file(path2senspos=path2file, delimiter=delimiter,
                                               hdr_scanpos_id=hdr_scanpos_id, hdr_x=hdr_x, hdr_y=hdr_y, hdr_z=hdr_z,
                                               sens_pos_id_offset=sens_pos_id_offset)
                ...
                with tqdm(total=file.header.point_count, desc="Tracing Pulses...", unit="pulses") as pbar:
                    for points in file.chunk_iterator(points_per_iteration=self.points_per_iter):
                        ...
                        if self.is_mobile:
                            ...
                            SensorPos = interpolate_traj(self.traj['time'], self.traj['sensor_x'],
                                                         self.traj['sensor_y'],
                            ...
                            SensorPos = pd.DataFrame(
                                data={'ScanPos': np.ones(gps_time.shape) * self.senspos['ScanPos'].values[0],
                        ...
                        if np.max(number_of_returns) == 1 or np.max(return_number) == 1:
                            run_raytraycing_after_loading = False
                            self.RayTr.doRaytracing_singleReturnPulses(x, y, z, SensorPos['sensor_x'],
                                                                       SensorPos['sensor_y'],
    ...
    def save_raytracing_output(self):
        ...
        self.Classification[
            np.logical_and.reduce((self.Nhit > 0, self.Nmiss >= 0, self.Nocc >= 0))] = 1  # voxels that were observed
        self.Classification[
            np.logical_and.reduce((self.Nhit == 0, self.Nmiss > 0, self.Nocc >= 0))] = 2  # voxels that are empty
        self.Classification[
            np.logical_and.reduce(
                (self.Nhit == 0, self.Nmiss == 0, self.Nocc > 0))] = 3  # voxels that are hidden (occluded)
