...
def normalize_occlusion_grids(nhit_grid, nmiss_grid, nocc_grid, classification_grid, PlotDim, vox_dim, dtm_file, output_folder=None, dsm_file=None, lower_threshold=0, output_voxels=False):
    """
        normalize_occlusion_grids normalizes all occpy grids (Nhit, Nmiss, Nocc, Classification) with the specified DTM
        ...
        Parameters
        ----------
        nhit_grid : numpy array (3D)
            NHit grid from occpy output
        nmiss_grid : numpy array (3D)
            NMiss grid from occpy output
        nocc_grid : numpy array (3D)
            NOcc grid from occpy output
        classification_grid : numpy array (3D)
            Classification grid from occpy output
        PlotDim : list
        ...
        output_folder: str, optional
            output folder where the normalized grids will be saved as .npy files. If None, the normalized grids will not be saved.
    ...
    if pix_size != vox_dim:
        ...
        if output_folder is not None:
            ofile = os.path.join(output_folder, f"{dtm_fname[:-4]}_resc_{vox_dim}.tif")
        else:
            # just write in the same folder as the input dtm file
            ofile = os.path.join(os.path.dirname(dtm_file), f"{dtm_fname[:-4]}_resc_{vox_dim}.tif")
        dtm.crop2extent(
    ...
    if dsm_file is not None:
        ...
        if pix_size != vox_dim:
            ofile = None
            if output_folder is not None:
                ofile = os.path.join(output_folder, f"{dsm_fname[:-4]}_resc_{vox_dim}.tif")
            else:
                # just write in the same folder as the input dsm file
                ofile = os.path.join(os.path.dirname(dsm_file), f"{dsm_fname[:-4]}_resc_{vox_dim}.tif")
            dsm.crop2extent(
                extent=extent_voxgrid,
                out_file=ofile,
                res=vox_dim)
        ...
        for y in range(0, dsm.shape[0], 1):
            for x in range(0, dsm.shape[1], 1):
                ...
                prof_class = classification_grid[x, y, zind_dtm:zind_dsm]
                prof_class_buf = classification_grid[x, y,
                ...
                Nhit_norm[x, y, 0:len(prof_class)] = nhit_grid[x, y, zind_dtm:zind_dsm]
                Nmiss_norm[x, y, 0:len(prof_class)] = nmiss_grid[x, y, zind_dtm:zind_dsm]
                Nocc_norm[x, y, 0:len(prof_class)] = nocc_grid[x, y, zind_dtm:zind_dsm]
        ...
        Nhit_norm = np.zeros(nhit_grid.shape, dtype=int)
        Nmiss_norm = np.zeros(nmiss_grid.shape, dtype=int)
        Nocc_norm = np.zeros(nocc_grid.shape, dtype=int)
        Classification_norm = np.zeros(classification_grid.shape, dtype=int)
        ...
        for y in range(0, dtm.shape[0], 1):
            for x in range(0, dtm.shape[1], 1):
                # get zind where DTM is located in grid at x,y
                zind_dtm = int(np.floor((dtm[y, x] - PlotDim[2]) / vox_dim))
                zind_dsm = last_nonzero(nhit_grid[x, y, :], axis=0)
                ...
                Classification_norm[x, y, 0:len(prof_class)] = classification_grid[x, y, zind_dtm:zind_dsm]
                Nhit_norm[x, y, 0:len(prof_class)] = nhit_grid[x, y, zind_dtm:zind_dsm]
                Nmiss_norm[x, y, 0:len(prof_class)] = nmiss_grid[x, y, zind_dtm:zind_dsm]
                Nocc_norm[x, y, 0:len(prof_class)] = nocc_grid[x, y, zind_dtm:zind_dsm]
    ...
    if output_folder is not None:
        ...
        np.save(os.path.join(output_folder, "Nhit_norm.npy"), Nhit_norm)
        np.save(os.path.join(output_folder, "Nmiss_norm.npy"), Nmiss_norm)
        np.save(os.path.join(output_folder, "Nocc_norm.npy"), Nocc_norm)
        np.save(os.path.join(output_folder, "Classification_norm.npy"), Classification_norm)

    # write ply file TODO: This seems to not be working for me!
    if output_voxels and output_folder is not None:
        print(f"Saving normalized output files into directory as .ply...")
        tic = time.time()
        verts, faces = prepare_ply(vox_dim, PlotDim, Nhit_norm)
        ost.write_ply(os.path.join(output_folder, "Nhit_norm.ply"), verts, ['X', 'Y', 'Z', 'data'], triangular_faces=faces)
        verts, faces = prepare_ply(vox_dim, PlotDim, Nmiss_norm)
        ost.write_ply(os.path.join(output_folder, "Nmiss_norm.ply"), verts, ['X', 'Y', 'Z', 'data'], triangular_faces=faces)
        verts, faces = prepare_ply(vox_dim, PlotDim, Nocc_norm)
        ost.write_ply(os.path.join(output_folder, "Nocc_norm.ply"), verts, ['X', 'Y', 'Z', 'data'], triangular_faces=faces)
        verts, faces = prepare_ply(vox_dim, PlotDim, Classification_norm)
        ost.write_ply(os.path.join(output_folder, "Classification_norm.ply"), verts, ['X', 'Y', 'Z', 'data'],
...
def normalize_occlusion_output(input_folder, PlotDim, vox_dim, dtm_file, dsm_file=None, lower_threshold=0, output_voxels=False):
    """
    normalize_occlusion_output normalizes the grids in an occpy output directory with the specified DTM by calling normalize_occlusion_grids on the output grids.
    This function also calculates occlusion statistics for the total canopy volume (defined by the volume between DTM
    and DSM). Currently only binary occlusion is analysed at the moment (TODO: implement also fractional occlusion),
    i.e. only voxels that are completely occluded (Nhit==0 and Nmiss==0 and Nocc >0)
    ...
    return normalize_occlusion_grids(Nhit, Nmiss, Nocc, Classification, PlotDim, vox_dim, dtm_file, output_folder=input_folder, dsm_file=dsm_file, lower_threshold=lower_threshold, output_voxels=output_voxels)
